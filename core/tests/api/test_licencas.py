...
The numbers are written by hand in this file. A test that asked the map for
...
# The numbers, written by hand on purpose. Product av first, product ar after.
...
def _em_disco(amb) -> dict:
    """The file as written, with the key of each licence opened by the vault."""
    dados = json.loads((amb.dir_data / ARQUIVO_CONFIG).read_text(encoding="utf-8"))
    chaveiro = cofre.abrir(amb.dir_data)
    for licenca in dados.get("licencas", []):
        assert not licenca["chave"] or cofre.cifrado(licenca["chave"]), "chave written in clear"
        licenca["chave"] = chaveiro.decifrar(licenca["chave"])
    return dados
...
async def test_um_hub_sem_licenca_lista_os_dois_produtos(abrir):
    """Zero licences is a normal state, and the panel reads the two products and
    the report policy from here instead of carrying a copy.
    """
...
async def test_cada_numero_carrega_os_data_points_da_secao_8(abrir):
    """The panel reads the numbering from here, so it never carries a copy."""
    cliente, auth = await abrir({TIPO: _fabrica()}, licencas=(LICENCA_AV, LICENCA_AR))
...
async def test_a_chave_e_gravada_e_nunca_devolvida(abrir, amb):
    """The chave is the credential of the device on the platform, so the answer
...
async def test_o_produto_de_uma_licenca_nunca_muda(duas, amb):
    """The product decides the table and the numbers already assigned; a
...
async def test_apagar_a_licenca_leva_os_numeros_e_deixa_o_equipamento(duas, amb):
    """Removing a licence empties its numbers without removing equipment."""
    cliente, auth, _classe = duas
...
async def test_o_qr_leva_o_uuid_e_o_pid_e_nunca_a_chave(abrir):
    """The QR code carries what the app needs to pair and never the credential."""
...
async def test_uma_ordem_maior_que_o_contrato_e_recusada(abrir, licenca, cabem):
    """Numbers twelve equipment of audio and video and eight machines of air, and
...
async def test_a_mesma_caixa_nao_ocupa_numero_em_duas_licencas(abrir, amb):
    ...
    # The file carries the order of every licence of the book, an empty one included.
...
async def test_um_ar_condicionado_so_entra_numa_licenca_de_ar_e_uma_caixa_so_numa_de_av(abrir, amb):
    """The product of a licence is decided by the category of the manifest, in
    both directions.
    """
    cliente, auth = await abrir(
...
async def test_perfis_que_nao_cabem_nas_cinco_strings_sao_recusados(abrir, amb):
    """The profiles of a licence share five strings of 255 bytes, and an order that
    does not pack is refused with the integrator at the keyboard instead of taking every
    profile off the bus.
    """
    # Ten inputs of sixteen characters make a profile of about 190 bytes, which fits the
...
async def test_o_snapshot_traz_o_reportavel_e_nunca_o_que_e_so_envio(duas):
    """The chip never echoes, so a scene and a command are never reported, and an
...
async def test_um_set_de_data_point_de_report_nao_chega_a_caixa(duas):
    """A report is only ever born of real state, so 144 takes no set."""
    cliente, auth, classe = duas
    resposta = await cliente.post(
...
# Str.isdigit is true for the superscript two and int refuses it, so this path used
...
async def test_o_ligado_de_um_equipamento_always_on_e_nao_suportado(duas):
    """A speaker that declares neither power capability has no switch, so the
...
async def test_o_ligado_de_um_projetor_chega_como_ligar_e_desligar(abrir):
    """Any registered equipment occupies a number, so a projector saved in number
...
async def test_uma_palavra_que_a_maquina_nao_fala_e_recusada_antes_do_driver(ar, dpid, valor):
    """A mode of the enum that the manifest does not declare never
...
async def test_o_canal_de_comando_chega_pela_rota_como_uma_capacidade(duas):
    """The panel writes n:acao[:valor] and the hub turns it into one capability
...
async def test_o_dp_de_cena_responde_o_mesmo_que_a_rota_de_cenas(duas):
    """DP 141 runs a scene, so it answers the codes of the scene executor and not
...
async def test_o_dp_de_cena_de_uma_licenca_de_ar_e_a_mesma_cena(ar):
    """The same number is the same scene in every licence,."""
    cliente, auth, _classe = ar
...
async def test_o_grupo_se_forma_pelo_mestre_e_cai_pelo_mestre(duas):
    """The slave joins the master, and only the master takes the group down."""
    cliente, auth, classe = duas
...
async def test_um_grupo_de_um_numero_que_o_contrato_nao_tem_e_recusado(duas, valor):
    """DP 142 carries the NUMBER of the master, and numbers twelve of them."""
    cliente, auth, classe = duas
...
async def test_uma_licenca_de_ar_nao_forma_grupo(abrir):
    """The group is of the licence of audio and video; a machine of air has no
...
async def test_tirar_o_mestre_da_ordem_derruba_o_grupo_pelo_mestre(duas):
    """The group is taken down while the OLD order still reaches the master;
...
async def test_um_numero_escravo_de_grupo_alheio_nao_e_desenhado_como_solo(duas):
    ...
    licenca = await _licenca(cliente, auth)
    # A slave of a group this hub does not lead has no master the hub could route to, so
...
async def test_um_corpo_json_fundo_demais_e_corpo_invalido_e_nunca_erro_interno(duas):
    ...
    """
    cliente, auth, _classe = duas
    # The scenes route takes a body of 256 kB, so a body deep enough to exhaust the
...
async def test_trocar_o_tipo_para_o_outro_produto_com_numero_e_recusado(abrir, amb):
    """An equipment only enters a licence of its product, so a speaker on number 1
...
async def test_a_rota_de_acao_de_um_escravo_passa_pelo_mestre(duas):
    """The volume and the transport of a slave go through the master, so the
    ...
    assert escravo.chamadas == []
    # An action the group does not route still reaches the equipment itself.
    resposta = await cliente.post(
...
async def test_a_rota_de_grupo_leva_os_membros_escolhidos(duas):
    """The panel picks who follows the master one by one, so the route carries the
