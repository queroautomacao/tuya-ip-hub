...
a driver is tested against a fake server and never against hardware, and this one
...
def _perfil(modos: list[str] | None = None, ventos: list[str] | None = None) -> str:
    # The real profile of a unit, measured on 7/set/2026, wraps every resource under one
    # "property" key; the simulated one used to answer the resources at the top level, and
    # the suite stayed green while the real unit refused every mode and every fan speed.
    # The simulated cloud answers like the real one now.
    return _resposta(
        {
            "property": {
                "airConJobMode": {
...
async def test_toda_requisicao_leva_o_token_a_chave_de_api_e_o_pais(nuvem):
    """The headers of the cloud, without any one of them the account answers 401."""
    async with ServidorHttp(_rotas()) as servidor:
        driver = nuvem(servidor)
        await driver.atualizar()
    ...
    # The message id is per request, so two of them are never the same.
...
async def test_um_aparelho_desligado_recusa_todo_comando_menos_ligar(nuvem):
    """With airConOperationMode in POWER_OFF the unit takes nothing, so a scene
...
async def test_o_modo_e_o_vento_usam_as_palavras_que_a_unidade_declara(nuvem):
    """The spelling of a model comes from its profile, and a word the unit does
...
async def test_uma_temperatura_fora_do_contrato_nunca_chega_a_nuvem(nuvem, valor):
    """Fixes whole degrees from 16 to 30, and True is an int in Python."""
    async with ServidorHttp(_rotas()) as servidor:
        driver = nuvem(servidor)
        await driver.atualizar()
...
def test_a_base_e_https_e_nunca_vem_do_cadastro():
    """The host is a constant of the driver, so no registration can send the token
...
async def test_o_parear_adota_o_unico_ar_da_conta_e_o_entrega_para_gravar(nuvem):
    """An account with ONE air conditioner has no ambiguity to resolve.

    Demanding an id that exists only inside the account, before the first call to the
    account, is demanding what nobody has yet; the integrator was copying it out of a log to
    paste it back into the field beside it.
    """
    sem_id = _Cadastro(campos={"pais": PAIS, "dispositivo": ""})
    async with ServidorHttp(_rotas(**{DEVICES: _conta("um-so")})) as servidor:
        driver = nuvem(servidor, sem_id)
        assert await driver.autenticar() == "pareado"
    assert driver.credenciais_do_aparelho() == {"dispositivo": "um-so"}


async def test_com_dois_ares_a_escolha_e_do_integrador_e_nada_e_adotado(nuvem):
    """With two the choice is his, and the log is where the ids are."""
    sem_id = _Cadastro(campos={"pais": PAIS, "dispositivo": ""})
    async with ServidorHttp(_rotas(**{DEVICES: _conta("um", "outro")})) as servidor:
        driver = nuvem(servidor, sem_id)
        assert await driver.autenticar() == "falhou"
    assert driver.credenciais_do_aparelho() == {}


async def test_um_cadastro_com_id_nao_adota_nada(nuvem):
    """An id the integrator wrote is his choice and pairing never replaces it."""
    async with ServidorHttp(_rotas(**{DEVICES: _conta(DISPOSITIVO, "outro")})) as servidor:
        driver = nuvem(servidor)
        assert await driver.autenticar() == "pareado"
    assert driver.credenciais_do_aparelho() == {}


async def test_o_profile_real_de_uma_unidade_da_os_modos_e_os_ventos(nuvem):
    """The words of the profile a real unit answered on 7/set/2026, read whole."""
    modos = ["HEAT", "COOL", "AIR_DRY", "FAN", "AUTO"]
    ventos = ["LOW", "MID", "HIGH", "AUTO"]
    async with ServidorHttp(_rotas(**{PERFIL: _perfil(modos, ventos)})) as servidor:
        driver = nuvem(servidor)
        await driver.atualizar()
        assert await driver.executar("modo", "auto") is None
        assert await driver.executar("modo", "seco") is None
        assert await driver.executar("vento", "medio") is None
        assert await driver.executar("vento", "auto") is None


async def test_um_modo_em_que_a_nuvem_recusa_o_setpoint_deixa_de_oferecer_um(nuvem):
    """Measured on 7/set/2026: in auto the cloud answers 2305 "Command not supported in AUTO"
    to a setpoint. The press comes back as the error of the device, and from then on the
    state offers no setpoint in that mode, until the mode changes.
    """
    modos = ["HEAT", "COOL", "AIR_DRY", "FAN", "AUTO"]
    recusa = json.dumps({"error": {"message": "Command not supported in AUTO", "code": "2305"}})
    em_auto = _estado(airConJobMode={"currentJobMode": "AUTO"})
    async with ServidorHttp(_rotas(**{PERFIL: _perfil(modos), ESTADO: em_auto})) as servidor:
        driver = nuvem(servidor)
        await driver.atualizar()
        assert (driver.estado().modo, driver.estado().temperatura) == ("auto", 22)
        servidor.rotas[CONTROLE] = (400, recusa)
        assert await driver.executar("temperatura", 23) == "erro_aparelho"
        assert driver.estado().temperatura is None
        await driver.atualizar()
        assert driver.estado().temperatura is None
        servidor.rotas[CONTROLE] = (200, _resposta({}))
        assert await driver.executar("modo", "frio") is None
        assert driver.estado().temperatura == 22
        assert await driver.executar("modo", "auto") is None
        assert driver.estado().temperatura is None
