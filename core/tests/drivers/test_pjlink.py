...
"""The example native driver against a simulated projector, no hardware.

The poll of one atualizar, and what it reads, has a file of its own beside this one.

The tests attack the contract as much as they exercise the protocol: an action
...
def test_o_manifesto_e_valido_e_nao_promete_volume():
    """What class 1 cannot do is omitted, never implemented to refuse."""
    ...
    exigidas = {"descricao", "preparo", "campo_porta", "campo_senha", "auth_ajuda"}
    for idioma in ("pt", "en"):
...
def test_a_senha_e_um_campo_segredo_e_o_ip_nao_e_campo():
    ...
    # Security off means no password, and demanding one would block a working setup.
...
def test_os_codigos_do_driver_sao_os_estaveis_do_contrato():
    """A code invented here would be a phrase the panel cannot translate,."""
    usados = {
        pjlink.EQ_OFFLINE,
...
async def test_autenticar_responde_pareado_ou_falhou_e_nunca_aguardando(
    ...
    """And this protocol has no popup on the device, so it never waits."""
...
async def test_o_mudo_do_painel_nunca_apaga_a_imagem_do_projetor():
    """Mudo is the audio one: 31 and 30 blank the picture, and nobody asked for that."""
...
def test_o_texto_do_manifesto_diz_que_o_mudo_e_de_audio():
    """What the panel shows about the driver comes from textos, in both languages."""
