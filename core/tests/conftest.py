...
def amb(tmp_path: Path, dir_painel: Path) -> Ambiente:
    # No mDNS in the suite: a test must not join the multicast group of the machine it runs on.
    return Ambiente(
        bind="127.0.0.1", porta=8080, dir_data=tmp_path / "data", dir_painel=dir_painel, mdns=False
    )
...
def agenda():
    ...
    reread), and both are attacked in the tests, so neither is really slept:
