...
"""Over the equipment routes: the session, the ip literal and the device secret.
...
async def test_origin_de_outro_site_e_403_em_toda_rota_de_equipamento(com_dono, metodo, caminho):
    # The session lives in the browser of the integrator, so a page of the attacker
...
async def test_a_rota_de_cadastro_recusa_um_ip_que_nao_e_literal(com_dono, texto):
    # A name would make the hub resolve and reach whatever the caller wrote,
...
async def test_o_segredo_do_aparelho_nunca_volta_em_resposta_nenhuma(com_dono):
    ...
    # The removal goes last, so every other route answers about an equipment that is
...
async def test_o_segredo_digitado_na_chave_errada_do_config_nao_sai(
    ...
    # A config.json edited by hand can carry the password in campos, and the answer of
...
async def test_o_segredo_de_um_tipo_que_saiu_da_imagem_nao_sai_por_rota_nenhuma(
    ...
    # With no manifest nothing says which key of the registration is a credential, and
...
async def test_acao_fora_das_capacidades_nao_chega_ao_driver(com_dono):
    # The gate refuses before the driver is touched, so no driver ever writes
...
async def test_identidade_no_caminho_nao_alcanca_outra_rota(com_dono):
    # The identity is a uuid, a MAC or a serial written by whoever holds a session, and
