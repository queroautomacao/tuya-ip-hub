...
"""An Origin that is not this host is 403 on /api/ and on /dpbus (closes CSRF)."""
...
async def test_a_acao_nao_acontece_por_tras_do_403(cliente, senha):
    # The claim is public, so a page of another site loaded by whoever is on the network
...
async def test_origin_de_outra_porta_do_mesmo_nome_e_403(cliente):
    # The browser treats another port as another origin, and so does the gate.
...
async def test_sem_origin_passa(cliente):
    # A tool sends no Origin and a browser always sends one, so the absent header is not
...
async def test_o_painel_e_o_health_nao_dependem_do_origin(cliente):
    # The gate guards what changes state or hands out data of the installation; the
