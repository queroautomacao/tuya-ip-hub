...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("GET", "/"),
        ("GET", "/assets/app.js"),
        # A path with no route and a method with no route used to escape to aiohttp,
...
async def test_expect_100_continue_segue_para_a_rota(cliente):
    # The client library does not model an interim 100, so the exchange goes over a socket.
