...
"""Over the licence and scene routes, with every test attacking a rule.
...
class Projetor(Driver):
    """Not a multiroom equipment: it takes a number like any other, never a
    group.
    """
...
async def test_nenhuma_rota_de_licenca_responde_a_outro_site(hub, amb, metodo, caminho):
    """A present Origin that is not this host is 403, which closes CSRF."""
    cliente, auth = hub
    resposta = await cliente.request(
...
async def test_a_chave_da_licenca_nunca_volta_em_resposta_nenhuma(hub, amb):
    """The chave is the credential of the device on the platform, so it is
    written and never read back, by no route of the daemon, the public one included.
    """
    cliente, auth = hub
    # The file really holds the credential, otherwise this test would prove nothing.
    ...
    pedidos = (
        ...
        # The removal goes last, so every other route answers about a licence that is
...
async def test_um_passo_de_cena_nao_roda_o_que_ninguem_roda(hub, amb, acao):
    """A step is one capability or the group, and a report is only
