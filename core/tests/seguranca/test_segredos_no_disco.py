...
"""Every file with a secret is 0600, and no secret ever leaves in a response."""
...
async def test_trocar_a_senha_rotaciona_o_api_token(cliente, posse, senha, bearer, amb):
    ...
    # The machine credential is what the DP-bus authenticates with, so a password change
...
async def test_o_token_de_sessao_nao_e_gravado_em_claro(cliente, posse, amb):
    ...
    # Whoever reads the file must not come out of it with a session; a fingerprint is
...
async def test_o_diretorio_de_dados_nao_se_abre_para_outro_usuario(fabrica_cliente, amb, senha):
    # This directory holds the four files; created with the umask of the
...
def test_link_no_lugar_do_token_nao_entrega_arquivo_de_fora(amb):
    # A link planted at api-token.txt would make the daemon read any file it can open and
    ...
    # Refusing to read through the link must not turn into writing through it either.
