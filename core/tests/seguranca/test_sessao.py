...
"""The panel session, its idle validity, its absolute cap and its revocations."""
...
async def test_o_teto_mata_a_sessao_mesmo_em_uso_constante(cliente_relogio, posse, bearer, relogio):
    ...
    # A token stolen from a browser that never closes would otherwise live forever.
...
async def test_trocar_a_senha_revoga_todas_as_sessoes(cliente, posse, senha, bearer):
    ...
    # The password is changed exactly when a session may be in the wrong hands, so no
...
async def test_sessao_criada_antes_da_posse_nao_existe(cliente, bearer):
    # No route may hand a token before the password exists, so nothing can be presented.
...
async def test_authorization_com_bytes_fora_do_utf8_e_sessao_invalida(cliente, metodo, caminho):
    ...
    # Those bytes reach the handler as lone surrogates, and hashing one raised out of the
