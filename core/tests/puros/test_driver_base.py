...
"""Under attack: the base must refuse, not pretend, and never publish a dict."""
...
async def test_autenticar_herdado_estoura_em_vez_de_fingir(auth):
    """The base REFUSES the inherited default, it never answers pareado."""
...
def test_defina_troca_o_estado_inteiro_e_preserva_o_resto():
    ...
    # A reader that kept the old object must not see it change under its hands.
...
def test_defina_recusa_campo_que_o_estado_nao_tem():
    """A field the panel would read has to be born in Estado, with a test,."""
