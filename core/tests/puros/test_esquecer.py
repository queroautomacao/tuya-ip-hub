...
"""Forgetting the password from the data directory, and what it must not take."""
...
def test_esquecer_mantem_a_instalacao_inteira(dir_data: Path):
    # Erasing config.json would clear the password and take the equipment, the licences,
...
def test_esquecer_mata_as_sessoes_e_troca_a_credencial_de_maquina(dir_data: Path):
    """Whoever could not get in must not keep a credential issued before."""
    _com_dono(dir_data)
