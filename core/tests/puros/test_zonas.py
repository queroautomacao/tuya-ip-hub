...
read as playing, a removal must never move the speaker of zone 2 into zone 1, and two group
...
stop precisa ler como tocando, uma remoção nunca pode mover a caixa da zona 2 para a zona 1,
...
from iphub.dpbus.zonas import Zonas
...
async def _nunca(_segundos: float) -> None:
    """The poll loop parks here: this file attacks the zones, never the schedule.

    O laço de poll estaciona aqui: este arquivo ataca as zonas, nunca o agendamento.
    """
...
async def duas(monta):
    ...
    return gestor, Zonas(gestor, ("uuid-1", "uuid-2"))
...
async def test_a_ordem_so_aceita_identidade_ja_cadastrada(duas):
    """Section 6: there is no second registry, so a zone names an equipment that exists.

    Seção 6: não existe segundo cadastro, então uma zona nomeia um equipamento que existe.
    """
    _gestor, zonas = duas
    with pytest.raises(modulo.OrdemInvalida) as erro:
        await zonas.definir_ordem(["uuid-1", "uuid-que-ninguem-cadastrou"])
    assert erro.value.codigo == "eq_nao_encontrado"
    assert zonas.ordem == ("uuid-1", "uuid-2")


async def test_um_equipamento_que_nao_e_multiroom_nao_ocupa_um_bloco(monta):
    ...
    zonas = Zonas(gestor)
    with pytest.raises(modulo.OrdemInvalida) as erro:
        await zonas.definir_ordem(["uuid-1", "uuid-projetor"])
    assert erro.value.codigo == "eq_nao_multiroom"


async def test_multiroom_sem_a_capacidade_de_agrupar_nao_ocupa_um_bloco(monta):
    """The manifest decides and nothing else: no agrupar, no zone.

    O manifesto decide e nada mais: sem agrupar, sem zona.
    """
    gestor = await monta({TIPO: _fabrica(capacidades=("volume", "tocar"))}, (_cadastro("uuid-1"),))
    with pytest.raises(modulo.OrdemInvalida) as erro:
        await Zonas(gestor).definir_ordem(["uuid-1"])
    assert erro.value.codigo == "eq_nao_multiroom"


async def test_um_setimo_bloco_nao_existe(duas):
    """Section 8 numbers six blocks, and a seventh identity names a zone the bus has not.

    A seção 8 numera seis blocos, e uma sétima identidade nomeia uma zona que o barramento
    não tem.
    """
    _gestor, zonas = duas
    with pytest.raises(modulo.OrdemInvalida) as erro:
        await zonas.definir_ordem(["uuid-1", "", "", "", "", "", "uuid-2"])
    assert erro.value.codigo == "zonas_demais"


async def test_a_mesma_caixa_em_dois_blocos_e_recusada(duas):
    """One speaker answering the volume of two zones is a device that contradicts itself.

    Uma caixa respondendo o volume de duas zonas é um aparelho que se contradiz.
    """
    _gestor, zonas = duas
    with pytest.raises(modulo.OrdemInvalida) as erro:
        await zonas.definir_ordem(["uuid-1", "uuid-1"])
    assert erro.value.codigo == "zona_repetida"
...
async def test_remover_um_equipamento_esvazia_a_vaga_e_nao_move_ninguem(duas):
    """Section 8: the position IS the block, so a removal must never promote zone 2.

    A shift would move the speaker of zone 2 into zone 1 in every automation the customer
    already built, and nothing on the bus would say it happened.

    Seção 8: a posição É o bloco, então uma remoção nunca pode promover a zona 2.

    Empurrar moveria a caixa da zona 2 para a zona 1 em toda automação que o cliente já
    montou, e nada no barramento diria que isso aconteceu.
    """
    gestor, zonas = duas
    await gestor.remover("uuid-1")
    assert await zonas.esquecer("uuid-1") == ("", "uuid-2")
    assert zonas.identidade(2) == "uuid-2"
    assert zonas.bloco("uuid-2") == 2
...
async def test_a_numeracao_do_bloco_e_a_da_secao_8(monta):
    """The whole numbering of section 8 for three zones, written by hand in the test.

    A numeração inteira da seção 8 para três zonas, escrita na mão no teste.
    """
    gestor = await monta(
        {TIPO: _fabrica()},
        tuple(_cadastro(f"uuid-{n}", ip=f"192.0.2.{n}") for n in (1, 2, 3)),
    )
    zonas = Zonas(gestor, ("uuid-1", "uuid-2", "uuid-3"))
    valores = zonas.valores()
...
async def test_um_bloco_vazio_nao_publica_data_point_nenhum(monta):
    """A bridge reading a false online would show an empty zone as a speaker switched off.

    Uma ponte lendo um online falso mostraria uma zona vazia como caixa desligada.
    """
    gestor = await monta({TIPO: _fabrica()}, (_cadastro("uuid-2", ip=IP_2),))
    valores = Zonas(gestor, ("", "uuid-2")).valores()
...
async def test_uma_identidade_pendurada_e_um_bloco_vazio(duas):
    ...
    assert await zonas.aplicar(VOLUME_1, 30) == "zona_offline"
...
async def test_a_entrada_publicada_e_so_uma_que_a_caixa_declara(duas):
    ...
    is not published as the input of that zone.

    Seção 14: só as entradas que o hardware declara existem, então um modo que ninguém
    declarou não é publicado como a entrada daquela zona.
    """
    gestor, zonas = duas
    _caixa(gestor, "uuid-1").estado_de_teste(fonte="hdmi3", fontes=("wifi", "line-in"))
    assert ENTRADA_1 not in zonas.valores()


async def test_o_que_a_zona_toca_vira_o_play_e_o_tocando(duas):
...
async def test_a_zona_que_toca_sem_titulo_ainda_reporta_o_play(duas):
...
async def test_a_zona_que_nao_sabe_dizer_nao_reporta_o_play(duas):
...
async def test_um_grupo_misto_nunca_e_oferecido(monta):
    ...
    zonas = Zonas(gestor, ("uuid-1", "uuid-outra"))
    assert await zonas.aplicar(GRUPO, "grupo1") == "nao_suportado"
    assert _chamadas(gestor, "uuid-outra") == []
    assert zonas.grupo() == "solo"
    assert zonas.valores()[NOMES_GRUPOS] == json.dumps({"g": ["", ""]}, separators=(",", ":"))
...
async def test_o_transporte_de_um_escravo_vai_para_o_mestre(duas):
    ...
    """
    gestor, zonas = duas
    await zonas.aplicar(GRUPO, "grupo1")
    _caixa(gestor, "uuid-2").chamadas.clear()
    assert await zonas.aplicar(PLAY_2, True) is None
    assert await zonas.aplicar(PLAY_2, False) is None
...
async def test_o_preset_de_um_escravo_vai_para_o_mestre(duas):
    ...
    await zonas.aplicar(GRUPO, "grupo1")
    _caixa(gestor, "uuid-2").chamadas.clear()
    assert await zonas.aplicar(PRESET_2, "cmd3") is None
    assert _chamadas(gestor, "uuid-2") == []
...
async def test_o_volume_de_um_escravo_passa_pelo_mestre(duas):
    ...
    await zonas.aplicar(GRUPO, "grupo1")
    _caixa(gestor, "uuid-2").chamadas.clear()
    assert await zonas.aplicar(VOLUME_2, 42) is None
    assert _chamadas(gestor, "uuid-2") == []
    ...
    assert await zonas.aplicar(VOLUME_1, 30) is None
...
async def test_a_entrada_de_um_escravo_nao_e_desviada_para_o_mestre(duas):
    ...
    assert await zonas.aplicar(ENTRADA_2, "line-in") == "nao_suportado"
...
async def test_desfazer_o_grupo_fala_com_o_mestre_e_solta_todo_mundo(duas):
    ...
    assert await zonas.aplicar(GRUPO, "solo") is None
    assert _chamadas(gestor, "uuid-1") == [("desfazer_grupo", None)]
    assert zonas.grupo() == "solo" and zonas.escravos() == ()
...
async def test_o_escravo_que_saiu_do_modo_multiroom_e_reconciliado(duas):
    ...
    _caixa(gestor, "uuid-1").chamadas.clear()
    await zonas.sincronizar()
    assert zonas.escravos() == ()
    assert zonas.grupo() == "solo"
    assert _chamadas(gestor, "uuid-1") == [("desfazer_grupo", None)]
    # The volume of that zone goes back to the zone itself, and not through the old master.
    # O volume daquela zona volta para a própria zona, e não pelo mestre antigo.
    assert await zonas.aplicar(VOLUME_2, 55) is None
...
async def test_o_grupo_cai_quando_o_mestre_sai_da_ordem(duas):
    ...
    gestor, zonas = duas
    await zonas.aplicar(GRUPO, "grupo1")
    await zonas.definir_ordem(["", "uuid-2"])
    assert zonas.grupo() == "solo"
    assert _chamadas(gestor, "uuid-2") == [("entrar_no_grupo", IP_1)]
...
async def test_as_operacoes_de_grupo_sao_serializadas(duas):
    ...
    trocar = asyncio.create_task(zonas.aplicar(GRUPO, "grupo2"))
    desfazer = asyncio.create_task(zonas.aplicar(GRUPO, "solo"))
    for _ in range(8):
...
async def test_set_em_bloco_vazio_e_recusado(monta):
    ...
    zonas = Zonas(gestor)
    assert await zonas.aplicar(VOLUME_1, 30) == "zona_offline"
    assert await zonas.aplicar(GRUPO, "grupo1") == "zona_offline"


@pytest.mark.parametrize("dpid", [ONLINE_1, TOCANDO_1, NOMES_ZONAS, NOMES_GRUPOS])
async def test_set_em_data_point_de_reporte_e_recusado(duas, dpid):
    ...
    """
    _gestor, zonas = duas
    assert await zonas.aplicar(dpid, True) == "dp_somente_leitura"
...
async def test_um_data_point_que_nao_e_deste_modulo_e_recusado(duas, dpid):
    ...
    """
    _gestor, zonas = duas
    assert await zonas.aplicar(dpid, "cena1") == "dp_desconhecido"
...
async def test_um_valor_fora_do_tipo_do_dp_nunca_chega_a_caixa(duas, dpid, valor):
    ...
    """
    gestor, zonas = duas
    assert await zonas.aplicar(dpid, valor) == "valor_invalido"
    assert _chamadas(gestor, "uuid-1") == []
...
async def test_o_codigo_do_driver_vira_o_vocabulario_do_barramento(duas, resposta, esperado):
    ...
    codigo = await zonas.aplicar(VOLUME_1, 30)
...
async def test_um_movimento_de_grupo_com_codigo_proprio_vira_erro_aparelho(duas):
    ...
    assert await zonas.aplicar(GRUPO, "grupo1") == "erro_aparelho"
    assert zonas.grupo() == "solo"
...
async def test_nenhuma_excecao_escapa_do_aplicar(duas):
    ...
    assert await zonas.aplicar(VOLUME_1, 30) == "erro_aparelho"
    assert await zonas.aplicar(GRUPO, "grupo1") == "erro_aparelho"
...
async def test_um_driver_multiroom_sem_os_movimentos_de_grupo_nao_derruba_o_barramento(monta):
    ...
    zonas = Zonas(gestor, ("uuid-1", "uuid-2"))
    assert await zonas.aplicar(GRUPO, "grupo1") == "zona_offline"
    await zonas.sanear()


async def test_nomes_longos_nao_tiram_os_nomes_das_zonas_do_barramento(monta):
    ...
    zonas = Zonas(gestor, tuple(f"uuid-{n}" for n in range(1, 7)))
    valores = zonas.valores()
    for dpid in (NOMES_ZONAS, NOMES_GRUPOS):
        texto = valores[dpid]
        assert len(texto.encode("utf-8")) <= 255
        assert len(json.loads(texto)[{NOMES_ZONAS: "z", NOMES_GRUPOS: "g"}[dpid]]) == 6
...
async def test_um_nome_que_o_json_escapa_nao_tira_os_nomes_do_barramento(monta, caractere):
    ...
    names of all six zones off the bus.
    ...
    valores = Zonas(gestor, tuple(f"uuid-{n}" for n in range(1, 7))).valores()
    for dpid, chave in ((NOMES_ZONAS, "z"), (NOMES_GRUPOS, "g")):
        texto = valores[dpid]
        assert len(texto.encode("utf-8")) <= 255
...
async def test_um_nome_acentuado_nunca_e_cortado_no_meio_de_um_caractere(monta):
    ...
    texto = Zonas(gestor, tuple(f"uuid-{n}" for n in range(1, 7))).valores()[NOMES_ZONAS]
    assert len(texto.encode("utf-8")) <= 255
    assert all(nome.startswith("Área") for nome in json.loads(texto)["z"])


async def test_os_nomes_das_zonas_seguem_a_ordem(duas):
    _gestor, zonas = duas
    assert zonas.valores()[NOMES_ZONAS] == json.dumps(
        {"z": ["Sala", "Cozinha"]}, ensure_ascii=False, separators=(",", ":")
    )


def test_a_config_carrega_as_zonas_e_recusa_o_que_a_secao_8_nao_numera(tmp_path):
    ...
    dir_data.mkdir()
    config.salvar(Config(zonas=("uuid-1", "", "uuid-2")), dir_data)
    assert config.carregar(dir_data).zonas == ("uuid-1", "", "uuid-2")
...
def test_zonas_vazias_sao_o_padrao_de_um_hub_sem_caixa():
...
@pytest.mark.parametrize(("zona", "valor"), [(0, "solo"), (1, "grupo1"), (6, "grupo6")])
def test_o_grupo_da_zona_e_o_nome_no_fio(zona, valor):
    assert modulo.valor_do_grupo(zona) == valor
    assert modulo.zona_do_grupo(valor) == zona


@pytest.mark.parametrize("valor", ["grupo0", "grupo7", "grupo", "", None, 1, "SOLO"])
def test_um_grupo_que_a_secao_8_nao_nomeia_nao_e_zona(valor):
    assert modulo.zona_do_grupo(valor) is None
...
async def test_sanear_pergunta_as_seis_caixas_juntas_e_nao_uma_apos_a_outra(monta):
    ...
    zonas = Zonas(gestor, tuple(f"uuid-{n}" for n in range(1, 7)), limite_s=0.2)
...
async def test_uma_caixa_escrava_de_grupo_alheio_e_reconhecida_e_nao_lida_como_solo(duas):
    ...
    nothing here put it there. Reading it as solo left the zone dead on the bus with the panel
    ...
    entrada, e nada aqui a pôs lá. Lê-la como solo deixava a zona morta no barramento com o
    painel desenhando controles que só respondem não, sem nada dizendo por quê.
    """
    gestor, zonas = duas
    assert zonas.escravos_alheios() == ()
    _caixa(gestor, "uuid-1").escravo_alheio = True
    assert zonas.escravos_alheios() == (1,)
    ...
    await zonas.aplicar(GRUPO, "grupo1")
    assert 2 not in zonas.escravos_alheios()
...
async def test_um_ungroup_recusado_nao_esquece_o_grupo_e_a_repeticao_tenta_de_novo(duas):
    ...
    assert await zonas.aplicar(GRUPO, "solo") == "zona_offline"
    assert zonas.grupo() == "grupo1", "the books forgot a group that is still physically up"
    mestre.recusa = None
    mestre.chamadas.clear()
    assert await zonas.aplicar(GRUPO, "solo") is None
...
async def test_um_escravo_largado_dos_livros_perde_a_marca_de_grupo(duas):
    """A zone dropped from the books while its driver still believes it is grouped refuses
    transport and input forever, for a group nobody is in any more.

    Uma zona largada dos livros com o driver dela ainda achando que está agrupada recusa
...
async def test_a_caixa_presa_em_grupo_alheio_e_convidada_a_sair_no_boot_e_no_sincronismo(duas):
    ...
    input, so leaving it there is leaving the zone dead on the bus.
    ...
    to leave and every command of that zone failed forever.

    Uma caixa presa num grupo que este hub não lidera recusa volume, transporte, preset e
    entrada, então deixá-la ali é deixar a zona morta no barramento.
    ...
    que saísse e todo comando daquela zona falhava para sempre.
    """
    gestor, zonas = duas
