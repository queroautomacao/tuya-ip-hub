...
"""Under attack: every rule of the manifest gets a manifest that breaks it."""
...
def test_o_produto_e_o_template_nascem_da_categoria():
    """An air conditioner is the product of air; a TV or a projector draws the
...
# A capability spoken in words (a key, a mode, a fan speed) is declared with its words,
# so the panel and the command channel offer only what the driver really sends.
...
def test_assinatura_de_descoberta_que_nao_e_tupla_de_texto_nao_embarca():
    """A broken signature must fail at load, not when the sweep plan is built."""
...
def test_um_driver_sugere_itens_para_a_lista_que_uma_capacidade_dele_le():
    """What a driver suggests fills a list of the registration, so it is judged by
