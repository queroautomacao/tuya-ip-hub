...
O diário do hub: um anel das últimas linhas, e nada do que ele faz pode quebrar quem loga.
"""
...
def test_o_anel_guarda_as_ultimas_linhas_e_conta_o_que_soltou():
    ...
    Um hub roda por meses, então o diário guarda as últimas linhas e diz quantas descartou; um
    buraco lido como silêncio mandaria o integrador procurar um aparelho que nunca emudeceu.
    """
    diario = Diario(limite=3)
    for numero in range(5):
        diario.emit(_registro(mensagem=f"linha {numero}"))
    assert [linha.mensagem for linha in diario.linhas()] == ["linha 2", "linha 3", "linha 4"]
    assert diario.descartadas == 2
    diario.limpar()
    assert diario.linhas() == ()
    assert diario.descartadas == 0
...
def test_uma_mensagem_enorme_ou_com_controle_nao_quebra_a_linha():
    ...
    """
    diario = Diario()
    diario.emit(_registro(mensagem="a" * (MENSAGEM_MAXIMA * 3)))
    diario.emit(_registro(mensagem="quebra\nde\x00linha"))
    primeira, segunda = diario.linhas()
...
def test_uma_mensagem_que_nao_formata_vira_linha_e_nunca_excecao():
    ...
    diario.emit(registro)
    assert "unformattable" in diario.linhas()[0].mensagem
...
def test_a_excecao_de_uma_falha_entra_na_linha_sem_o_traceback():
    ...
    A última linha de um traceback é o que um diário carrega; o log do container guarda o resto.
    """
    diario = Diario()
    try:
    ...
    diario.emit(registro)
    assert diario.linhas()[0].mensagem == "falhou [ValueError: valor ruim]"
...
def test_instalar_ve_o_debug_do_daemon_e_nao_instala_dois():
    ...
    O diário tem nível próprio, então o painel vê todo comando que um driver escreveu enquanto
    ...
    try:
        primeiro = instalar()
        segundo = instalar()
        assert [alvo for alvo in raiz.handlers if isinstance(alvo, Diario)] == [segundo]
...
def test_o_json_de_uma_linha_e_o_que_o_painel_le():
    diario = Diario()
    diario.emit(_registro(nome="iphub.dpbus.socket", mensagem="set dp 121 = 30"))
    como_json = diario.linhas()[0].como_json()
