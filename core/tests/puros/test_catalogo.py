...
"""Under attack: nobody edits a list by hand and a broken manifest never ships."""
...
from iphub.drivers.manifesto import TEXTO_PREPARO, Manifesto, ManifestoInvalido, validar
...
def test_dois_modulos_com_o_mesmo_tipo_e_erro_de_teste(fabrica_pacote):
    """An ambiguity is caught in the suite, never resolved at runtime."""
    pacote = fabrica_pacote(
...
def test_todo_driver_da_imagem_diz_o_que_ligar_no_aparelho_antes():
    """Rule 11 of the bench, what has to be turned on IN THE DEVICE before it
    answers is the first thing an integrator needs, so every shipped driver writes it, in both
    languages, and the panel shows it before the fields.
    """
    for tipo, classe in catalogo.carregar().items():
        for idioma in ("pt", "en"):
            preparo = classe.MANIFESTO.textos[idioma].get(TEXTO_PREPARO, "")
            assert len(preparo) >= 40, f"{tipo}: {idioma} has no preparo worth reading"
