...
def test_dockerfile_roda_como_usuario_nao_root_no_estagio_final(instrucoes):
    # Only the last stage ships; a USER in the panel stage proves nothing.
...
def test_compose_constroi_na_rede_do_host():
    # The reference ARM appliance has no bridge network for build containers either.
    compose = _ler("docker-compose.yml")
...
def test_compose_repete_os_tempos_do_healthcheck_da_imagem():
    # A healthcheck in compose replaces the image's entirely, boot slack included.
    compose = _ler("docker-compose.yml")
    for campo, valor in (
        ("start_period", "45s"),
        ("interval", "10s"),
...
def test_dockerfile_nao_da_a_arvore_do_codigo_ao_usuario_do_servico(instrucoes):
    # /app is the working directory, so it lands first on sys.path; a writable /app
...
def test_imagem_carrega_o_aviso_de_licenca_do_painel(instrucoes):
    # The bundle embeds React and the minifier drops its notice, so the notice ships apart.
