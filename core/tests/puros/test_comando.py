...
"""Under attack: the command channel is a closed grammar, and everything outside it
...
a value refuses one, an index is two digits at most, a key is a word and an
...
def test_o_vocabulario_do_canal_e_o_da_secao_8():
    ...
    # Every word of the channel is one capability, and nothing outside it.
...
def test_a_tecla_e_uma_palavra_de_teclas_e_nada_mais():
    """The panel speaks the words of TECLAS and the driver translates each one, so
