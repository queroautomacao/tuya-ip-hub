...
panel for a case that never happens. The rules being attacked are the ones (a
step is one action on one registered equipment, thirty two scenes whose
position is the number, the 255 BYTES of each of the two name data points) and the rule
 fixes for a driver and holds for a scene: no condition, no loop, no expression, no
...
# Something outside Exception, which is what a device library raising SystemExit deep
...
# Three registered pieces of equipment, named by identity and never by IP.
...
def test_a_cena_quebrada_e_recusada_pelo_campo_e_nada_mais(rotulo, dados, esperado):
    ...
    # A registry that knows every identity a file names adds no problem of its own, so
...
def test_uma_recusa_e_um_value_error():
    # Whoever refuses to boot on a broken file catches ValueError, and config turns this
...
def test_validar_nunca_estoura_outra_coisa_alem_de_cenas_invalidas(dados):
    # This judges a hand edited config.json on boot, and anything else leaving here
...
def test_o_valor_de_um_passo_e_julgado_pela_acao(acao, valor):
    """Says what each action takes, and the value travels to the driver untouched."""
...
def test_toda_acao_de_cena_tem_um_valor_aceito():
    # An action that the table above never accepts would be a scene nobody can write.
...
def test_a_posicao_de_uma_cena_e_o_numero_dela(acionador):
    """Numbers 1 to 32, so an erased scene empties its slot instead of pulling the
