...
"""Under attack: the numbering of the two products is written here by hand and the
code has to match.
...
the report class the policy reads.
"""
...
# The product of air, copied: machine k starts at 101 + 5(k - 1).
AR = {
    ...
    **{104 + 5 * (k - 1): (k, "vento") for k in range(1, 9)},
    ...
    175: (0, "nomes_cenas"),
    180: (0, "acesso_remoto"),
    181: (0, "acesso_remoto_restante"),
    182: (0, "acesso_remoto_codigo"),
    183: (0, "acesso_remoto_url"),
    184: (0, "acesso_remoto_home"),
}
...
def test_os_produtos_e_os_numeros_sao_os_do_documento():
    ...
    assert CENAS == 32
    # The five of the remote door are on both products and on the same numbers.
    assert len(mapa.tabela(PRODUTO_AR)) == 42
    assert len(mapa.tabela(PRODUTO_AV)) == 44
...
# The report policy reads the class of every data point, so a data point
