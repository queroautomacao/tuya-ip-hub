...
"""The sweep plan is generated from the manifests, and two types that claim the
same signature are an error of this suite, never a decision taken in runtime.

 in spirit: an answer is data. The address of a device is where the datagram came
...
@pytest.fixture
async def inundador():
    """Opens the device of the attack : one that answers without stopping."""
    transportes = []
...
def test_dois_tipos_no_mesmo_fabricante_e_erro_desta_suite():
    # The substring is compared in lower case, so a difference of case is the same claim.
    manifestos = [
...
def test_o_plano_do_catalogo_embarcado_leva_o_que_a_imagem_declara():
    """The rule aimed at what actually ships, not only at hand built manifests."""
...
def test_um_driver_novo_que_repete_assinatura_do_catalogo_embarcado_e_ambiguo():
    """Pointed at the image plus one driver more, which is how a clash arrives."""
    embarcados = _embarcados()
    sts_embarcados = tuple(st for m in embarcados for st in m.descoberta.ssdp_st)
    # The image ships no SSDP signature today, so vizinho gives the shipped set one to
...
async def test_o_ip_e_o_remetente_nao_o_que_o_location_aponta(rude):
    """The attack : an answer that names another host must not move the address."""
...
async def test_a_varredura_para_no_teto_de_achados(inundador, monkeypatch, caplog):
    """One device on the segment must not decide how big an answer gets."""
...
def test_um_uuid_de_dois_enderecos_nao_substitui_o_primeiro(caplog, tipo_do_impostor):
    """The attack : answering with the uuid of the projector in the next room.
