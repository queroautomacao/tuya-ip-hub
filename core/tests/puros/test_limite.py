...
"""Five failures per IP block for fifteen minutes, sixty attempts a minute.
...
def test_ataque_distribuido_bate_no_teto_global(limite):
    # One failure per IP never reaches the per IP block, so only the global window
...
def test_o_relogio_padrao_e_o_monotonico():
    # The counters live in memory only, so a backwards step of the wall clock (NTP on a
...
def test_a_tentativa_que_nao_e_registrada_nao_gasta_a_janela(limite):
    # A request refused before the credential is checked costs no PBKDF2, so it may not
...
def test_consultar_o_bloqueio_tambem_poda(limite, relogio):
    # An attacker that rotates addresses stops adding keys only when every entry point
...
def test_o_proxy_que_anexa_entrega_o_ultimo_salto():
    # The common reverse proxy appends what it saw, so position zero is text the client
...
def test_o_ipv6_encaminhado_vira_forma_canonica():
    # Two spellings of one address would be two keys in the block map, that is two blocks
