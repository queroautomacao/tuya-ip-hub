...
"""A file under the data directory is born 0600 and is written atomically."""
...
def test_temporario_tambem_nasce_0600(tmp_path: Path, umask_aberto, monkeypatch):
    # Between the open and the replace the secret already exists on disk; a temporary
...
def test_diretorio_de_dados_nasce_0700(tmp_path: Path, umask_aberto):
    # This directory holds every secret; on a host with umask 022 it would
...
def test_escrita_sincroniza_o_diretorio_depois_da_troca(tmp_path: Path, monkeypatch):
    # The appliance goes down by losing power; without the directory fsync the rename
