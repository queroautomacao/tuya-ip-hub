...
# Something outside Exception, which is what a device library raising SystemExit or a
...
async def test_uma_falha_nao_derruba_duas_derrubam_e_um_sucesso_levanta(monta):
    """Generalized: one lost answer is a hiccup, two in a row are an offline device."""
    classe = _fabrica()
    bomba = Bomba()
...
async def test_o_que_o_aparelho_disse_nunca_vira_detalhe(monta):
    """Detalhe is one code of DETALHES, so a device line never reaches the screen."""
...
async def test_o_poll_e_escalonado_dentro_do_intervalo(monta):
    ...
    # One more sleep than steps, because a step parks the loop on the next one.
    assert bomba.dormidas == [5.0] * 5


async def test_o_hub_funciona_com_zero_equipamentos(monta):
    """No equipment is a normal state, and the loop keeps its own pace."""
    bomba = Bomba()
...
async def test_cadastrar_e_remover_com_o_laco_rodando(monta):
    ...
    # Three polls, because the registration is visited on the spot and then twice by the
...
async def test_driver_travado_no_poll_nao_segura_o_poll_do_proximo(monta):
    ...
    # Without the deadline the pump never comes back, so the test hangs instead of
