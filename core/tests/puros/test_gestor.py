...
"""Under attack: the gate refuses before the driver and one bad driver stays alone."""
...
async def test_acao_fora_das_capacidades_nunca_chega_ao_driver(monta):
    """The manifest refuses, so no driver writes a method only to say no."""
...
async def test_driver_que_nao_responde_no_prazo_e_eq_offline_e_nao_erro_aparelho(monta):
    """Of the stable codes, a device that did not answer in time is eq_offline.

    The deadline is the gestor's and it fires before the driver's own whenever the
...
async def test_codigo_inventado_pelo_driver_vira_erro_aparelho(monta, resposta):
    """A code of the driver's own would reach the panel as an untranslated phrase."""
...
async def test_cadastro_de_tipo_desconhecido_sobrevive_e_e_reportado_offline(monta):
    ...
    # The tipo is a name the integrator typed and it would reach the screen
...
async def test_driver_que_estoura_no_estado_nao_derruba_a_listagem_dos_outros(monta):
    """The gestor is the enforcer of estado(), so one bad driver stays alone."""
...
async def test_todo_detalhe_que_o_gestor_publica_e_um_codigo_do_vocabulario(monta):
    """Detalhe is empty or one code of DETALHES, never a phrase to read."""
...
async def test_trocar_catalogo_refaz_so_o_tipo_nomeado(monta):
    """A driver that was saved is built again, and every other keeps its session."""
    ...
    # Rebuilding a driver nobody asked about would drop the connection of every device on
...
async def test_trocar_o_driver_nao_abre_sessao_por_cima_do_poll_em_voo(monta):
    """A matrix and a projector accept ONE connection at a time, so the driver
