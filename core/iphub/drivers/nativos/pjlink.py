...
"""PJLink class 1 over TCP, the example native driver.
...
log = logging.getLogger("iphub.drivers.nativos.pjlink")

PORTA_PADRAO = 4352
TEMPO_LIMITE_S = 4.0
# One poll asks up to four questions, so a deadline that is only per exchange lets a
# projector that greets and then goes silent hold its slot for four times the limit.
ORCAMENTO_DO_POLL_S = 6.0
# A device on the LAN must not be able to make the daemon buffer without bound, so an
...
# Warm up is on its way to lit and cool down is on its way to dark, so each one reports
...
# The mute is the audio one, so 21 and 20 are written and the picture is never
...
TEXTOS = {
    "en": {
        ...
        "preparo": (
            "Turn network control (PJLink) on in the menu of the projector, and only if you want a "
            "password turn PJLink security on and write the password down. Port 4352 from the "
            "factory. A projector in standby still answers, so it can be turned on from here."
        ),
    ...
    "pt": {
        ...
        "preparo": (
            "Ligue o controle por rede (PJLink) no menu do projetor e, só se quiser senha, ligue a "
            "segurança PJLink e anote a senha. Porta 4352 de fábrica. Um projetor em standby "
            "continua respondendo, então ele liga daqui."
        ),
...
class PJLink(Driver):
    ...
    # Class 1 has no volume command, and says the right move is to omit the
    ...
    async def autenticar(self) -> str:
        """Pairing is explicit. This protocol has no popup, so it never waits."""
        try:
    ...
    async def atualizar(self) -> None:
        ...
        # A projector that is off answers ERR3 to everything else, and reading that as a
    ...
    async def _trocar_fonte(self, valor: object) -> str | None:
        # The value decides bytes on a socket, so it is checked before the connection is
    ...
    async def _falar(self, comando: str, prazo_final: float | None = None) -> str:
        ...
        # An empty address would let the resolver aim the command at the loopback, which
        ...
        # A spent budget drops the questions the poll has left, before a socket is opened.
        if limite <= 0:
            raise _Falha(EQ_OFFLINE)
        try:
            async with asyncio.timeout(limite):
                ...
                try:
                    # The prefix is the digest of the password, so the transcript writes
                    # the command and never the prefix; a poll is one connection per question,
                    # so its questions are written once and its answers with them.
                    rotina = comando.endswith("?")
                    self._transcricao().enviado(comando, rotina=rotina)
                    prefixo = self._prefixo(await _linha(leitor))
                    escritor.write(f"{prefixo}{comando}{TERMINADOR}".encode("ascii"))
                    await escritor.drain()
                    resposta = await _linha(leitor)
                    self._transcricao().recebido(resposta, rotina=rotina)
                    return _valor(comando, resposta)
                finally:
            ...
            self._transcricao().falhou(comando, erro)
...
def _digesto(semente: str, senha: str) -> str:
    # This md5 is the PJLink handshake and not a secret at rest, and saying so keeps the
...
def _valor(comando: str, resposta: str) -> str:
    ...
    # A device answering another command is out of step with us, and nothing it says can
...
async def _fechar(escritor: asyncio.StreamWriter) -> None:
    escritor.close()
    # A cancellation of the poll must reach the caller, so only the noise of a peer
