...
"""The profile of an equipment of audio and video, the string the panel of the
...
# The name travels shortened to this many characters inside the profile.
NOME_MAXIMO = 20

# The letters, each one a control the panel draws when it is present.
LETRA_LIGAR = "L"
LETRA_NIVEL = "N"
LETRA_MUDO = "M"
LETRA_ENTRADA = "E"
...
# pause pair, stop, next. Each one is a capability of its own, so each one is a
...
# Which list of the registration each capability reads its values from.
...
def funcoes(cadastro: Cadastro, manifesto: Manifesto) -> str:
    """The letters in their fixed order, one per control the equipment offers."""
    capacidades = set(manifesto.capacidades)
    letras = []
    # Half of the power pair is no switch at all, because a switch that turns on and
    ...
    # Stop, previous and next have no opposite half to wait for, unlike the power pair
...
def cabe(perfil: str) -> bool:
    """True when the profile fits the ceiling."""
    try:
...
def _rotulos(cadastro: Cadastro, manifesto: Manifesto, capacidade: str) -> str:
    # A list the manifest cannot act on is not offered, because a button that only ever
