...
A driver for a new device is written as a JSON file, with no programming. The file says how to reach the device (a line of text on a TCP port, a simple HTTP request, or UDP), which commands it takes and how to read its state back. The panel opens a starting template per transport, validates the file field by field and loads it without restarting the hub; a file dropped by hand into the `drivers` directory of the data volume is loaded on the next boot and wins over an embedded driver of the same type. A file that does not validate is refused and logged by name: it never costs the boot, and every other driver goes on loading. The format is described in section 7 of [DECISOES.md](DECISOES.md).

Numbers on the app, scenes and the DP-bus arrived with milestone 4, and milestone 4b reshaped them around licences. A licence is one device on the Tuya platform, created on the Account screen of the panel with its uuid, pid and key (the key never comes back to the panel) and paired on the app by a QR code the panel draws. There are two products: an air conditioner licence carries up to eight machines (power, setpoint, mode, fan), an audio and video licence carries up to twelve equipment (power, level, plus a command channel for inputs, shortcuts, modes and keys the panel of the platform draws from a profile the hub publishes). Any registered equipment takes a number on a licence of its product from its own detail screen, a multiroom equipment groups with others of the same type there, a scene is a list of actions on equipment saved as data, with an interval between the steps, fired by the same number from any licence, and the bus is a WebSocket at `/dpbus` that authenticates on its first frame with the `api_token` and the licence, never from the URL, and reports only what changed, inside the daily budget of the platform. The data point scheme is public and written down in [DECISOES.md](DECISOES.md), so anybody may write a bridge on top of it. What is still missing are the native drivers of the receivers and the TVs and the published image, in the milestones below, one at a time, each one closed with a green CI.
...
| 4b | delivered | Two products and licences, scenes by equipment action, command channel, profiles, report policy, licences on the Account with QR | licence routes, bus per licence and scenes green; the two Tuya products are registered by hand |
...
Os números no app, as cenas e o DP-bus chegaram com o marco 4, e o marco 4b os reorganizou em torno de licenças. Uma licença é um dispositivo na plataforma Tuya, criada na tela Conta do painel com uuid, pid e chave (a chave nunca volta para o painel) e pareada no app por um QR code que o painel desenha. São dois produtos: uma licença de ar condicionado leva até oito máquinas (ligar, setpoint, modo, vento), uma licença de áudio e vídeo leva até doze equipamentos (ligar, nível, mais um canal de comando para entradas, atalhos, modos e teclas que o painel da plataforma desenha a partir de um perfil que o hub publica). Qualquer equipamento cadastrado ocupa um número numa licença do produto dele a partir do detalhe dele, um equipamento multiroom agrupa com outros do mesmo tipo ali, uma cena é uma lista de ações em equipamentos salva como dado, com um intervalo entre os passos, disparada pelo mesmo número de qualquer licença, e o barramento é um WebSocket em `/dpbus` que autentica no primeiro quadro com o `api_token` e a licença, nunca pela URL, e reporta só o que mudou, dentro do orçamento diário da plataforma. O esquema de data points é público e está escrito no [DECISOES.md](DECISOES.md), então qualquer um pode escrever uma ponte em cima dele. O que ainda falta são os drivers nativos dos receivers e das TVs e a imagem publicada, nos marcos abaixo, um por vez, cada um fechado com CI verde.
...
| 4b | entregue | Dois produtos e licenças, cenas por ação de equipamento, canal de comando, perfis, política de reports, licenças na Conta com QR | rotas de licença, barramento por licença e cenas verdes; os dois produtos Tuya são cadastrados à mão |
