...
// The address after the hash is the only state of navigation, so the back button, a
...
// The licence and the source belong on every page and matter to nobody in a hurry, so
...
function Rodape() {
  return (
    <footer className="rodape miudo">
      ...
      <p>
        {t("marca")} {t("marcas_fabricantes")} {t("copyright")}
      </p>
    </footer>
  );
}
...
function Painel({
  ...
  // The scenes and the simulator act on registered equipment, so their tabs open once one
...
export default function App() {
  ...
  const carregar = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      // A session that ended is the ordinary way back to the login screen,
  ...
  async function encerrar(): Promise<void> {
    try {
      ...
      // The browser has already dropped the token, so the screen goes back
