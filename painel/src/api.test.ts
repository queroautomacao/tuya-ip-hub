...
// Node has no localStorage, and api.ts reads the token on every request.
...
test("tomarPosse sends only the password (envia so a senha e guarda a sessao)", async () => {
  ...
  // The ownership code left so a body still carrying one would be the panel
...
test("a 401 senha_invalida keeps the live session (um 401 senha_invalida mantém a sessão viva)", async () => {
  ...
  // Mistyping the current password says nothing about the session, and signing
...
test("senhaCurta counts code points, as the daemon does (conta pontos de código, como o daemon)", () => {
  ...
  // Four astral characters are eight UTF-16 units and four code points, so
  ...
  // The count is code points and not grapheme clusters, because python len is
...
// Promises every refusal of a driver file at once, so the answer carries one
