...
// The customer never sees this panel; they see an app built on the data points of
// one device per licence. This screen draws that app inside a phone, over the same
...
interface Leitura {
  ...
  // The snapshot is the slice of ONE licence, so it travels with the id it belongs to
...
// The app of the customer polls its device a few times a minute, and the simulator
// wants to feel like it: three seconds is close to the report cadence.
...
export default function Simulador({ nomeInstalacao }: { nomeInstalacao: string }) {
  ...
  const recarregar = useCallback(async (): Promise<void> => {
    // A poll for the licence shown before may land after the poll for the one shown now,
