...
// A licence is a device on the platform: an identity the bridge uses, a QR
...
// written and never read back; the QR carries only the pid and the uuid.
...
function rascunhoDe(licenca: Licenca): Rascunho {
  // The chave never comes back from the daemon, so the edit form starts it blank and a
...
function Qr({ id, aoFechar }: { id: string; aoFechar: () => void }) {
  ...
  // The canvas is unmounted while an error shows and mounted again empty when the error
