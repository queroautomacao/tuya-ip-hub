...
// Says the ip of any route that talks to a device is an IP literal, so
...
test("a valid form sends only the declared fields, trimmed (um formulário válido envia só os campos declarados, aparados)", () => {
  ...
  const validacao = validarCadastro(
    formularioDeTeste({
      ...
      nome: "  Projetor  ",
      ip: "  192.0.2.10  ",
      // An empty secret is left out so the daemon keeps the credential it already
...
// The daemon stores a device credential verbatim, so trimming a password with a
...
test("a SEGREDO travels exactly as it was typed (um SEGREDO viaja exatamente como foi digitado)", () => {
  ...
  // A credential of nothing but spaces is still a credential the operator typed.
  const espacos = validarCadastro(formularioDeTeste({ campos: { senha: "   " } }), [item]);
  assert.equal(espacos.ok && espacos.corpo.campos.senha, "   ");
});

// The daemon refuses a text over 200 code points or with a control character with
...
test("the form enforces the limits of the daemon and names the field (o formulário impõe os limites do daemon e nomeia o campo)", () => {
  ...
  // The limit is in code points, the way python len counts, so an astral character
...
// An update that omits a secret keeps the one the daemon stores, so the operator
...
test("an edit keeps the stored credential and erases it only when asked (a edição mantém a credencial guardada e só a apaga quando pedem)", () => {
  ...
  // With nothing stored, an obligatory secret left blank is still a refusal.
...
// The identity is the key of a registration and the sweep is the only place it could
...
test("a cloud driver registers with no address and every other one still needs it (um driver de nuvem cadastra sem endereço e todo outro ainda o exige)", () => {
  // A device with no local API is reached through the cloud of its maker, so
...
test("the ceiling of the level is a number from 1 to 100 and names its field (o teto do nível é um número de 1 a 100 e nomeia o campo)", () => {
  assert.equal(nivelMaximo("100"), 100);
  assert.equal(nivelMaximo(" 40 "), 40);
  assert.equal(nivelMaximo("1"), 1);
  for (const errado of ["0", "101", "-5", "cem", "", "4.5"]) assert.equal(nivelMaximo(errado), null);
  const item = itemDe({ capacidades: ["volume"] });
  const base: Formulario = { ...VAZIO, tipo: item.tipo, identidade: "uuid-1", ip: "192.0.2.10" };
  const recusado = validarCadastro({ ...base, nivel_maximo: "150" }, [item]);
  assert.deepEqual(recusado, { ok: false, codigo: "campo_invalido", campo: "nivel_maximo" });
  const aceito = validarCadastro({ ...base, nivel_maximo: "60" }, [item]);
  assert.equal(aceito.ok && aceito.corpo.nivel_maximo, 60);
});
