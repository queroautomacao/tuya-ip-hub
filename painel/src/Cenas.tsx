...
// A scene is DATA, so this screen is an editor of a list and never a little language:
...
import Agendamentos from "./Agendamentos.tsx";
...
// The interval is typed over, so the field keeps what is being typed as a draft and only
...
function CampoIntervalo({
  ...
  return (
    <input
      ...
      onBlur={() => {
        setRascunho(null);
        // Leaving the field puts the last good value back on the screen, so the refusal
...
function Valor({
  ...
  if (especie === "grupo") {
    // A group only exists between equipment of the same tipo, and
  ...
  if (especie === "escolha") {
    ...
    // A value saved before the list changed is still the value of the step, so it stays
...
function Passo({
  ...
  const trocarEquipamento = (identidade: string): void => {
    // Moving a step to another equipment keeps what it does when that equipment offers
...
export default function Cenas() {
  ...
  useEffect(() => {
    void recarregar();
    // The cycle refreshes what is running, and a draft in the middle of being typed is
  ...
  // Thirty two cards is a wall; the screen shows the scenes in use plus one free slot, and
  ...
  return (
    <>
      ...
      {leitura.erro === null && leitura.maximo > 0 && <Agendamentos cenas={leitura.cenas} />}
