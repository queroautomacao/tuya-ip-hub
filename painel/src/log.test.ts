...
test("the clock of a line is the one of the record, to the millisecond (o relógio de uma linha é o do registro, ao milissegundo)", () => {
  // Two lines a hundred milliseconds apart are a command and its answer, so the
...
test("no origin chosen shows every origin (nenhuma origem escolhida mostra todas)", () => {
  // A filter that starts empty and shows nothing is a screen that looks broken the
  // moment it opens.
  ...
  assert.deepEqual(
    filtrar(linhas, ["tuya", "panel"], "").map((linha) => linha.origem),
    ["tuya", "panel"],
  );
});
