...
test("lerSnapshot reads the licence table as the daemon answers it (lê a tabela de licenças como o daemon responde)", () => {
  const item = {
    dpid: 142,
    numero: 0,
    indice: 0,
...
test("bitDe and paresDe read the packed reports (leem os reports empacotados)", () => {
...
test("controlesDoNumero draws only what the manifest and the lists offer (desenha só o que o manifesto e as listas oferecem)", () => {
  ...
  // The previous and next keys are their own capabilities, drawn only for a driver that
  ...
  // Half of the power pair is no switch, and a list the manifest cannot act on is not
