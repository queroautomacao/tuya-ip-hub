...
import { CHAVE_SESSAO, compartilhaOrigem, guardar, ler, limpar } from "./sessao.ts";

// Node has no localStorage, so the double is the global itself, put back
...
test("served from a sub path the panel shares its origin with other hubs (servido de um subcaminho o painel divide a origem com outros hubs)", () => {
  assert.equal(compartilhaOrigem("/"), false);
  assert.equal(compartilhaOrigem("/index.html"), false);
  assert.equal(compartilhaOrigem("/h/abcdefghijklmnopqrstuvwxyz/"), true);
  assert.equal(compartilhaOrigem("/h/abcdefghijklmnopqrstuvwxyz/index.html"), true);
});

test("through the relay the token lives in the storage of the tab, not of the origin (pelo relay o token vive no storage da aba, não da origem)", () => {
  const daOrigem = memoria();
  const daAba = memoria();
  const janela = { location: { pathname: "/h/abcdefghijklmnopqrstuvwxyz/" } };
  const antes = Object.getOwnPropertyDescriptor(globalThis, "window");
  const antesAba = Object.getOwnPropertyDescriptor(globalThis, "sessionStorage");
  Object.defineProperty(globalThis, "window", { configurable: true, value: janela });
  Object.defineProperty(globalThis, "sessionStorage", { configurable: true, value: daAba });
  try {
    comStorage(daOrigem, () => {
      guardar("token-de-fora");
      assert.equal(ler(), "token-de-fora");
      assert.equal(daAba.getItem(CHAVE_SESSAO), "token-de-fora");
      assert.equal(daOrigem.getItem(CHAVE_SESSAO), null);
      limpar();
      assert.equal(daAba.getItem(CHAVE_SESSAO), null);
    });
  } finally {
    if (antes) Object.defineProperty(globalThis, "window", antes);
    else delete (globalThis as unknown as Record<string, unknown>).window;
    if (antesAba) Object.defineProperty(globalThis, "sessionStorage", antesAba);
    else delete (globalThis as unknown as Record<string, unknown>).sessionStorage;
  }
});
