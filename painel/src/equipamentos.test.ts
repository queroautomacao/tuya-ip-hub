...
test("the panel of a speaker carries every transport key the driver declares (o painel de uma caixa leva toda tecla de transporte que o driver declara)", () => {
  // Stop is its own capability because a pause on a stream keeps the speaker
...
// A SEGREDO is a device credential that never leaves the daemon; if some answer
...
test("the driver labels come from the manifest, never from the panel (os rótulos do driver vêm do manifesto)", () => {
  ...
  // A manifest missing one language must show the other one, never the word
...
test("the catalog and the sweep are read against the contract (o catálogo e a varredura são lidos contra o contrato)", () => {
  ...
  // What the driver suggests fills a list of the registration, so a suggestion outside
  ...
  assert.deepEqual(lerAchado(achado), { ...achado, nome: "", ja_cadastrado: false });
...
// The API answers a stable code and never a human phrase, so a code with no entry
...
// Estado.detalhe carries the empty string or ONE code of a fixed vocabulary, so a
// phrase the daemon composed, or a device answer, has nothing to do on this screen; the
// panel refuses to print what it cannot translate.
...
// One transient failure of the catalog read used to leave the panel telling the
...
test("a failed catalog read is never an empty catalog (catálogo que falhou nunca é catálogo vazio)", () => {
  ...
  // A cycle that fails after a good one keeps what was read, so the screen does not
...
// The description of a sweep answer and the name of a device are text a stranger on
...
// Detalhe is a code like any other, so a member of the vocabulary with no phrase in
...
test("the home lists every brand of the catalog, once each and in order (o início lista toda marca do catálogo, uma vez cada e em ordem)", () => {
  // The brand of a driver is a fact of the manifest, so the screen reads it
  // from the catalog and keeps no list of its own that a new driver would leave stale; a
  // driver of no brand in particular (a generic protocol) simply does not appear.
  const catalogo = [
    itemDe({ tipo: "receiver_denon", marca: "Denon" }),
    itemDe({ tipo: "ar_lg_thinq", marca: "LG" }),
    itemDe({ tipo: "tv_lg_webos", marca: "LG" }),
    itemDe({ tipo: "multiroom_linkplay", marca: "LinkPlay" }),
    itemDe({ tipo: "generico", marca: "" }),
    itemDe({ tipo: "espaco", marca: "   " }),
  ];
  assert.deepEqual(marcasDoCatalogo(catalogo), ["Denon", "LG", "LinkPlay"]);
  assert.deepEqual(marcasDoCatalogo([]), []);
  // The same brand written in two cases is one brand, and the first spelling is the one kept.
  assert.deepEqual(
    marcasDoCatalogo([itemDe({ marca: "Sonos" }), itemDe({ marca: "SONOS" })]),
    ["Sonos"],
  );
});

test("the file of a logo is named by the slug of the brand (o arquivo de um logotipo é nomeado pelo apelido da marca)", () => {
  // The name of a brand is written by a person and carries case, accents and spaces,
  // while a file name does not; one rule turns one into the other so nobody has to guess the
  // spelling when dropping a logo into src/marcas.
  assert.equal(apelidoDa("LG"), "lg");
  assert.equal(apelidoDa("LinkPlay"), "linkplay");
  assert.equal(apelidoDa("  Denon  "), "denon");
  assert.equal(apelidoDa("Sony Bravia"), "sony-bravia");
  assert.equal(apelidoDa("Yamaha MusicCast"), "yamaha-musiccast");
  assert.equal(apelidoDa("Áudio & Vídeo"), "audio-video");
  assert.equal(apelidoDa(""), "");
});


test("a unit on, in a mode, with no temperature has no setpoint to offer (uma unidade ligada, num modo, sem temperatura não tem setpoint a oferecer)", () => {
  // The auto mode of an LG refuses a setpoint, so the driver answers none there, and
  // the card and the detail draw the same guard from the same three facts.
  assert.equal(semSetpoint({ ligado: true, modo: "auto", temperatura: null }), true);
  assert.equal(semSetpoint({ ligado: true, modo: "frio", temperatura: 22 }), false);
  assert.equal(semSetpoint({ ligado: false, modo: "auto", temperatura: null }), false);
  assert.equal(semSetpoint({ ligado: true, modo: null, temperatura: null }), false);
});

test("the suggested range is the /24 the panel was opened from, else the one of the registered equipment (a faixa sugerida é o /24 de onde o painel foi aberto, senão o dos equipamentos cadastrados)", () => {
  assert.equal(faixaSugerida("192.168.1.50", []), "192.168.1.0/24");
  assert.equal(faixaSugerida("localhost", ["192.0.2.10", "10.0.0.5", "10.0.0.9"]), "10.0.0.0/24");
  assert.equal(faixaSugerida("localhost", []), "");
  assert.equal(faixaSugerida("8.8.8.8", ["192.0.2.10"]), "");
  assert.equal(faixaSugerida("172.16.5.7", []), "172.16.5.0/24");
  assert.equal(faixaSugerida("localhost", [], ["192.168.1.0/24", "172.20.0.0/24"]), "192.168.1.0/24");
  assert.equal(faixaSugerida("localhost", ["10.0.0.5"], ["192.168.1.0/24"]), "10.0.0.0/24");
});

test("a find carries the name the network gave it, and the ones with a driver come first (um achado leva o nome que a rede lhe deu, e os com driver vêm primeiro)", () => {
  const base = { identidade: "", porta: null, descricao: "", ja_cadastrado: false };
  const lido = lerAchado({ ...base, tipo: "", ip: "192.0.2.7", nome: "tv-sala.local" });
  assert.equal(lido?.nome, "tv-sala.local");
  assert.equal(lerAchado({ ...base, tipo: "", ip: "192.0.2.7" })?.nome, "");
  const ordenados = ordenarAchados([
    { ...base, tipo: "", ip: "192.0.2.3", nome: "" },
    { ...base, tipo: "tv_roku", ip: "192.0.2.20", nome: "" },
    { ...base, tipo: "", ip: "192.0.2.10", nome: "" },
    { ...base, tipo: "receiver_yamaha", ip: "192.0.2.5", nome: "" },
  ]);
  assert.deepEqual(
    ordenados.map((achado) => achado.ip),
    ["192.0.2.5", "192.0.2.20", "192.0.2.3", "192.0.2.10"],
  );
});
